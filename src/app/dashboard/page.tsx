            ...
            <div className="w-full bg-slate-50 border-b border-slate-100">
                ...
                <div className="relative aspect-video w-full flex items-center justify-center bg-black">
                    {editForm.assets.length > 0 ? (
                        <>
                            {editForm.assets[0].type === 'video' ? (
                                <video src={editForm.assets[0].preview || (typeof editForm.assets[0].content === 'string' ? editForm.assets[0].content : '')} className="max-h-full max-w-full" controls />
                            ) : (
                                <img src={editForm.assets[0].preview || (typeof editForm.assets[0].content === 'string' ? editForm.assets[0].content : '')} alt="" className="max-h-full max-w-full object-contain" />
                            )}
                            <button
                                onClick={() => setEditForm({ ...editForm, assets: editForm.assets.filter((_, i) => i !== 0) })}
                                className="absolute top-4 right-4 p-2 bg-red-500/80 hover:bg-red-600 text-white rounded-full transition-colors z-10"
                                title="Remove Primary Asset"
                            >
                                <Trash2 size={16} />
                            </button>
                        </>
                    ...
                    {editForm.assets.map((asset, idx) => (
                        <div key={idx} className="shrink-0 w-24 h-24 rounded-lg overflow-hidden border border-slate-200 shadow-sm relative group">
                            {asset.type === 'video' ? (
                                ...
                                <img src={asset.preview || (typeof asset.content === 'string' ? asset.content : '')} alt="" className="w-full h-full object-cover" />
                            )}
                            <button
                                onClick={() => setEditForm({ ...editForm, assets: editForm.assets.filter((_, i) => i !== idx) })}
                ...
                <div className="w-full bg-slate-50 border-b border-slate-100">
                    ...
                    <div className="relative aspect-video w-full flex items-center justify-center bg-black">
                        {selectedItem.assets[0].type === 'video' ? (
                            <video src={selectedItem.assets[0].content as string} className="max-h-full max-w-full" controls />
                        ) : (
                            <img src={selectedItem.assets[0].content as string} alt="" className="max-h-full max-w-full object-contain" />
                        )}
                    </div>
                            ...
                            {selectedItem.assets.map((asset, idx) => (
                                <div key={idx} className="shrink-0 w-24 h-24 rounded-lg overflow-hidden border border-slate-200 shadow-sm relative group">
                                    {asset.type === 'video' ? (
                                        ...
                                        <img src={asset.content as string} alt="" className="w-full h-full object-cover" />
                                    )}
                                </div>
                            ))}
                        </div>
